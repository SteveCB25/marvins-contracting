...
async def populate_gallery_db(db):
    """Populate database with gallery images"""
    try:
        # Clear existing
        result = await db.gallery_images.delete_many({})
        ...
        return {
            ...
            "inserted": total,
            "breakdown": {k: len(v) for k, v in IMAGE_CATEGORIES.items()}
        }
    except Exception as e:
        logger.error(f"Error populating gallery: {str(e)}")
        raise

@router.post("/populate-gallery")
async def populate_gallery_endpoint(db):
    """One-time endpoint to populate gallery"""
    try:
        result = await populate_gallery_db(db)
        return result
    except Exception as e:
